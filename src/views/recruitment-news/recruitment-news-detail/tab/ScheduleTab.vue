<template>
    <div class="p-[16px] w-full h-full" style="background-color: #EBECEF;">
    ...
    <popup-detail-schedule v-if="isShowPopupDetail" :scheduleDetail="selectedScheduleDetail" :is-show-popup="isShowPopupDetail" @on-close="isShowPopupDetail = false" @on-save="handleSave()"/>
</template>
...
import PopupDetailSchedule from "../../../schedule/popup/PopupDetailSchedule.vue";
...
const emit = defineEmits(["showPopupAdd"]);
...
async function getSchedule(){
    ...
    pagingParam.SearchValue = searchValue.value
    pagingParam.Collums = [
        "CandidateName"
    ],
    ...
    pagingParam.CustomParam = {
        ...
        periodID: props.currentPeriod
    }
    const res = await candidateScheduleDetailApi.getScheduleDetailByRecruitment(pagingParam)
    if(res.data.Data){
        displayList.value = []
        listCandidateSchedule.value = res.data.Data
